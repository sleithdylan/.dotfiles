...
# ------------------------------------------------------------------------------
# Usage: chmod +x init.sh && ./init.sh [--update-packages]
# ------------------------------------------------------------------------------
...
install_apt_package() {
  ...
  if dpkg -l "$package" 2>/dev/null | grep -q "^ii"; then
    if [[ "$UPDATE_PACKAGES" == true ]]; then
      ...
      local output
      output=$(sudo apt-get install --only-upgrade -y "$package" 2>&1)
      if echo "$output" | grep -q "0 upgraded"; then
        log_warning "$package already at latest, skipping"
        SKIPPED+=("$package")
      else
...
install_brew_package() {
  ...
  if brew list "$package" &>/dev/null; then
    if [[ "$UPDATE_PACKAGES" == true ]]; then
      ...
      if echo "$output" | grep -qi "already installed\|up-to-date"; then
        log_warning "$package already at latest, skipping"
        SKIPPED+=("$package")
      else
        log_success "$package updated"
...
install_brew_cask() {
  ...
  if brew list --cask "$package" &>/dev/null; then
    if [[ "$UPDATE_PACKAGES" == true ]]; then
      log_info "Upgrading $package..."
      local output
      output=$(brew upgrade --cask "$package" 2>&1)
      if echo "$output" | grep -qi "already installed\|up-to-date"; then
        log_warning "$package already at latest, skipping"
        SKIPPED+=("$package")
      else
...
install_cargo_package() {
  ...
  if cargo install --list 2>/dev/null | grep -q "^$package "; then
    if [[ "$UPDATE_PACKAGES" == true ]]; then
      log_info "Upgrading $package via cargo..."
      if cargo install --force "$package" &>/dev/null; then
        log_success "$package updated"
        UPDATED+=("$package")
      else
...
setup_nvm() {
  ...
  if command -v nvm &>/dev/null; then
    ...
    for npm_pkg in pnpm yarn; do
      if npm list -g "$npm_pkg" &>/dev/null; then
        if [[ "$UPDATE_PACKAGES" == true ]]; then
          if npm update -g "$npm_pkg" &>/dev/null; then
            log_success "$npm_pkg updated"
            UPDATED+=("$npm_pkg")
          else
            log_error "$npm_pkg update failed"
            FAILED+=("$npm_pkg")
          fi
        else
          log_warning "$npm_pkg already installed, skipping"
          SKIPPED+=("$npm_pkg")
        fi
      else
        if npm install -g "$npm_pkg" &>/dev/null; then
          log_success "$npm_pkg installed"
          INSTALLED+=("$npm_pkg")
        else
          log_error "$npm_pkg failed"
          FAILED+=("$npm_pkg")
        fi
      fi
    done
...
show_summary() {
  ...
  if [[ ${#UPDATED[@]} -gt 0 ]]; then
    echo -e "${CYAN}Updated (${#UPDATED[@]}):${NO_COLOR}"
    echo -e "${GRAY}  ${UPDATED[*]}${NO_COLOR}"
    echo ""
  fi
